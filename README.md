...
**标签跳转功能** 让你可以通过标签快速定位并跳转到包含 `.index` 文件的目录，无需记忆或输入冗长的路径，尤其适合管理大量项目目录。
#### 前置知识
该功能基于标签系统的 `.index` 文件约定：在需要标记的目录下创建一个名为 `.index` 的文件，并为该文件添加自定义标签，即可通过标签快速跳转到该目录。
#### 基本语法
```bash
cd -idx <tag>        # 通过标签搜索目录并跳转
cd -idx <tag-regex>  # 支持正则表达式匹配标签
```
#### 使用示例
```bash
# 示例1：跳转到带有work标签的目录
cd -idx work
# 示例2：使用正则匹配，跳转到包含rust或project标签的目录
cd -idx "rust|project"
# 示例3：匹配多个标签的目录
cd -idx "work.*important"
```
#### 交互选择
当有多个目录匹配搜索的标签时，会显示带序号的选择列表：
```
...
输入对应的序号即可直接跳转到目标目录。
#### 核心特性
- **正则支持**：支持完整正则表达式语法匹配标签
- **多结果选择**：多个匹配结果时提供交互式选择列表
- **错误友好**：目标目录不存在或无法访问时提供清晰错误提示
- **支持两种运行模式**：在REPL交互模式和直接命令模式下均可使用
- **兼容现有功能**：跳转成功后会正确更新上一个目录历史，支持`cd -b`返回
#### 适用场景
- **多项目管理**：为每个项目目录添加标签，通过标签快速在不同项目间跳转
- **常用目录访问**：为经常访问的目录设置标签，无需输入路径
- **模糊搜索跳转**：记不清完整路径时，通过标签关键词快速定位目录
...
| `cd -idx <tag>`                             | 通过标签跳转到包含 `.index` 文件的目录，支持正则匹配 | `rfe cd -idx work`、`rfe cd -idx "rust.*project"`                        |
