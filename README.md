...
- 📍 **`{}.pop.pop...` 路径层级弹出**：在占位符后添加任意数量的 `.` 来向上回退目录层级，支持无限级联，到达路径边界时自动提示

## 🔑 核心概念与进阶特性

本节详细介绍 rfe 的核心语法特性：**`@` 路径别名**、**`->` 命令链式执行** 以及 **`{}.pop` 路径层级弹出**。理解这些概念将显著提升你在终端中的文件操作效率。
...
在处理路径时，有时你需要从当前路径向上回退到父目录。使用 `{}.pop` 语法，你可以在占位符后添加任意数量的 `.` 来表示向上回退的目录层级。每多一个 `.` 就代表向上回退一级目录。

#### 基本语法

```bash
cmd1 -> cd {}.pop           # 向上回退 1 级目录
cmd1 -> cd {}.pop.pop       # 向上回退 2 级目录
cmd1 -> cd {}.pop.pop.pop   # 向上回退 3 级目录
# ... 以此类推，可无限级联
```
...
# 示例 2：获取当前路径，切换到上两级目录（祖父目录）
rfe pwd -> cd {}.pop.pop

# 示例 3：复制文件路径，然后切换到该文件的父目录
rfe cpf src/main.rs -> cd {}.pop

# 示例 4：复制文件路径，然后切换到项目根目录（向上回退 3 级）
# 假设文件路径为 /project/src/main.rs，弹出 3 次后得到 /project
rfe cpf src/main.rs -> cd {}.pop.pop.pop

# 示例 5：结合别名使用，从深层目录快速跳转到别名目录的父级
rfe pwd -> cp {} -> alias add parent {}.pop

# 示例 6：边界情况演示 - 当弹出次数超过路径层级时
# 会停在最顶层并显示友好提示（不会报错）
rfe cpf file.txt -> cd {}.pop.pop.pop.pop.pop  # 只会弹出 1 次到空路径
```

#### 核心特性

- **任意层级弹出**：支持 `.pop`、`.pop.pop`、`.pop.pop.pop` 等任意级联，理论上没有限制
- **智能边界处理**：当弹出次数超过路径的实际层级时，会安全地停在最顶层，不会产生错误
- **友好边界提示**：到达路径边界时会显示提示信息，告诉你实际执行了多少次弹出
- **跨平台兼容**：完美支持 Windows 和 Unix/Linux 路径格式
- **与所有路径命令兼容**：可与 `cd`、`ls`、`open`、`mv` 等任何接收路径参数的命令配合使用

#### 适用场景

- **深层目录导航**：从深层嵌套的目录快速回退到指定的上级目录
- **文件到目录切换**：复制文件路径后，快速切换到该文件所在的目录或更上层目录
- **相对路径构造**：在命令链中基于前一个输出动态构造相对路径
- **项目根目录跳转**：从项目子目录快速跳转到项目根目录（如 `src/utils/mod.rs` → 连续 3 次 pop）
- **脚本化工作流**：在复杂的多步命令中动态调整路径层级
...
| `{}.pop.pop...`                             | 占位符路径层级弹出，每多一个 `.` 就向上回退一级目录 | `rfe pwd -> cd {}.pop.pop`（从当前路径向上回退 2 级）                   |
