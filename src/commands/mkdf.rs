...
pub fn cmd_mkdf(args: &[&str]) -> Result<(String, String), Box<dyn std::error::Error>> {
    ...
    if !create_file && !create_dir {
        return Err(
            "Please specify whether to create a file (-f/--file) or directory (-d/--directory)"
                .into(),
        );
    }
    ...
    if create_file {
        ...
        if parents {
            ...
            Ok((
                format!(
                    "{} Created directory (with parents): {}",
                    "✔".bright_green(),
                    target_path.display().to_string().cyan()
                ),
                target_path.display().to_string(),
            ...
            if let Some(parent) = target_path.parent() {
                if !parent.exists() {
                    return Err(format!(
                        "Parent directory does not exist. Use -p/--parents to create it: {}",
                        parent.display()
                    )
                    .into());
                }
