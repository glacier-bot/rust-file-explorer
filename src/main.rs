...
fn cmd_help() -> Result<(String, String), Box<dyn std::error::Error>> {
    ...
    output.push_str(&format!("  {}      只列出匹配指定标签的文件，支持正则和多标签组合\n", "ls -t/--tag <标签正则>".cyan().bold()));
    ...
    output.push_str(&format!("{}\n", "📝 常用正则表达式语法:".bright_yellow().bold()));
    output.push_str(&format!("  {}  匹配任意单个字符                          例: ls --re fi.e -> file, fine\n", ".".bright_cyan()));
    output.push_str(&format!("  {}  匹配前一个字符0次或多次                    例: ls --re a* -> a, aa, aaa\n", "*".bright_cyan()));
    output.push_str(&format!("  {}  匹配前一个字符1次或多次                    例: ls --re a+ -> a, aa, aaa\n", "+".bright_cyan()));
    output.push_str(&format!("  {}  匹配前一个字符0次或1次                     例: ls --re colou?r -> color, colour\n", "?".bright_cyan()));
    output.push_str(&format!("  {}  匹配字符串开头                            例: ls --re ^src -> src开头的文件\n", "^".bright_cyan()));
    output.push_str(&format!("  {}  匹配字符串结尾                            例: ls --re \\.rs$ -> 所有.rs文件\n", "$".bright_cyan()));
    output.push_str(&format!("  {}  匹配字符集中任意一个字符                    例: ls --re [Ff]ile -> File, file\n", "[abc]".bright_cyan()));
    output.push_str(&format!("  {}  匹配不在字符集中的任意字符                  例: ls --re [^Ff]ile -> aile, bile...\n", "[^abc]".bright_cyan()));
    output.push_str(&format!("  {}  或逻辑，匹配左右任意一个表达式              例: ls --re \\.rs$|\\.toml$ -> rs和toml文件\n", "|".bright_cyan()));
    output.push_str(&format!("  {}  分组，用于组合表达式                       例: ls --re (src|target)\\/ -> src或target目录下的文件\n\n", "()".bright_cyan()));
    ...
    output.push_str(&format!("{}\n\n", "✨ New Feature -> Path Aliases:".bright_green().bold()));
    ...
    output.push_str(&format!("{}\n\n", "✨ New Feature -> File Tags:".bright_green().bold()));
    ...
    output.push_str(&format!("    {}            列出当前目录下所有带rust标签的文件\n", "ls -t rust".cyan()));
    output.push_str(&format!("    {}  列出所有同时匹配rust和doc标签的文件\n", "tag find rust doc".cyan()));
    output.push_str("  支持正则表达式匹配，多标签组合查询，自动备份持久化\n\n");

    output.push_str(&format!("{}\n\n", "✨ New Feature -> Command Chain:".bright_green().bold()));
